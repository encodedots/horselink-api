    ...
    if (authHeader) {
        ...
        jwt.verify(token, process.env.JWT_SECRET_KEY, async (err, userData) => {
            if (err) {
                ...
                await userAuthTokens.update({ status: "n" }, { where: { token: token } });

                messages.authMessage = "Your token is expired, Please login to continue"
                return res.status(403).send(messages);
            } else {
                if (userData && userData.email) {
                    var tokenDetail = await userAuthTokens.findOne({ where: { token: token, email: userData.email, status: "y" } })
                    if (!tokenDetail) {
                        // Change token status when user not authorized
                        await userAuthTokens.update({ status: "n" }, { where: { token: token } });
