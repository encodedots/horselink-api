...
exports.getFileFromS3Bucket = function (input) {
    try {
        // Validate input data
        if (!input || !isValidString(input.key))
            return false;

        // Get file from AWS S3
        var params = {
            Bucket: (input.Bucket && isValidString(input.Bucket)) ? params.Bucket : process.env.BUCKET_NAME,
            Key: input.key
        };
...
exports.uploadFile = async function (input) {
    try {
        // Validate input data
        if (input == null || input.length < 1 || (input && !isValidString(input.file.name)))
            return false;
        ...
        var fileName = randomName + moment().format('YYYYMMDDhhmmss') + input.file.name.substring(input.file.name.lastIndexOf("."));
        const uploadParams = {
            Bucket: (input.Bucket && isValidString(input.Bucket)) ? params.Bucket : process.env.BUCKET_NAME,
            Body: fileStream,
...
exports.uploadFileFromPath = async function (input, fileName) {
    try {
        ...
        // Set parameter to upload file to s3 bucket
        let randomName = random(5, 'alphanumeric')
        var fileName = randomName + moment().format('YYYYMMDDhhmmss') + fileName.substring(fileName.lastIndexOf("."));
        const uploadParams = {
            Bucket: (input.Bucket && isValidString(input.Bucket)) ? params.Bucket : process.env.BUCKET_NAME,
            Body: input.file,
            Key: input.filePath ? input.filePath + fileName : fileName
...
exports.deleteFileFromS3 = async function (input) {
    try {
        // Validate input data
        if (!input || input.length < 1 || !isValidString(input.key))
            return false;

        // Set parameter to delete specific file
        var deleteParams = {
            Bucket: (input.Bucket && isValidString(input.Bucket)) ? input.Bucket : process.env.BUCKET_NAME,
            Key: input.key
        };
...
exports.validateImageSize = async function (imagePath, height, width) {
    ...
    // Validate input
    if (!isValidInteger(height) || !isValidInteger(width))
        return false;
