 ...
 */
exports.sendwithAttachmentImage = async function (sendTo, Subject, emailBody, Attachment) {
    try {
        const sendMail = await transporter.sendMail({
            from: process.env.SEND_FROM_EMAIL,
            from: 'HORSEMAPP<' + process.env.SEND_FROM_EMAIL + '>',
            to: sendTo,
            subject: Subject,
            html: emailBody,
 ...
 */
exports.generateHtmlForEmail = async function (sendEmailTemplateData, replaceEmailTemplateData, language = constants.DEFAULT_LANG) {

    let linkReplaceData = {
        siteUrl: process.env.SITE_REDIRECT_URL,
        loginUrl: process.env.SITE_LOGIN_REDIRECT_URL,
        contactUrl: process.env.SITE_CONTACT_REDIRECT_URL,
        instagramURL: process.env.INSTAGRAM_URL,
        contactAddress: "Horsemapp GmbH, <br>Gartenstr. 34, 82049 Pullach, Germany",
    }

    var moduleName = sendEmailTemplateData.moduleName
    let staticContent = myJson[language][moduleName]

    const emailHTML = await ejs.renderFile(
        process.env.EMAIL_TEMPLATE_LOCATION + sendEmailTemplateData.templateName,
        { replaceEmailTemplateData: replaceEmailTemplateData, linkReplaceData: linkReplaceData, staticContent: staticContent }
    );

    let attachments = [
        {
            filename: 'header_logo.png',
            path: process.env.EMAIL_TEMPLATE_IMAGE_LOCATION + 'images/header_logo.png',
